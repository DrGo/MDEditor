...
//import Yams    // For YAML parsing in tests
...
//        """
//        let document = Document(parsing: markdown)
//        let p = document.children.first(where: { $0 is Paragraph }) 
//        let ul = document.children.first(where: { $0 is UnorderedList }) as? UnorderedList
...
//        // Correctly find the UnorderedList within the ListItem's children
...
//
//        // Find the second top-level list more robustly
...
//        let ul0_alt = allTopLevelLists.count >= 2 ? allTopLevelLists[1] : nil
...
//
//        // Nested BlockQuote is often inside a Paragraph within the parent BlockQuote
//        var bq2: BlockQuote? = nil
//        if let firstChildOfBq1 = bq1?.children.first {
//            if let para = firstChildOfBq1 as? Paragraph {
